...
- [references/MIGRATION_STATEMENT_ORDERING.md](references/MIGRATION_STATEMENT_ORDERING.md) — SQL migrations on non-transactional runners (MySQL DDL, ledger-on-full-success): idempotent statements first, the single unguardable one last — applied to the down-file independently (NOT a mirror). Read when authoring or reviewing multi-statement migration pairs.
- [references/CONVERGENT_SEED_ROW_MIGRATION.md](references/CONVERGENT_SEED_ROW_MIGRATION.md) — load when a migration seeds a row a UI can also create (settings / theme-property / feature-flag / lookup registry): a guarded `INSERT … WHERE NOT EXISTS` is idempotent by **skip**, so a row an operator hand-created first is skipped **forever** and an applied file can never be edited to repair it. Pair it with an idempotent repair `UPDATE` covering every column a consumer *branches on* (the discriminator that picks the UI widget is the one most often missed), repairing shape while preserving the operator's value and prose.
