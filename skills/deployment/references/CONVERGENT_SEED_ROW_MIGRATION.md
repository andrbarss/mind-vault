...
**Match the code's comparator in every predicate.** `WHERE name = '…'` is case-insensitive under the usual collations, and without a UNIQUE key nothing stops a row that differs only in case from existing. If the code that reads (or hides, or authorises) the row identifies it **byte-exact**, a case-insensitive migration disagrees with it three times: the `NOT EXISTS` guard sees the variant and *skips the insert*, the repair `UPDATE` rewrites somebody else's custom row, and the down file deletes it. Use the code's comparator in all of them (`WHERE BINARY name = '…'` on MySQL; `BINARY <expr>` is deprecated from 8.0.27 — a warning today, `CAST(… AS BINARY)` when a server drops it), and add a **sixth verification case**: insert the case-variant row first → the up leaves it untouched and still inserts the exact row, the up SQL a second time changes nothing, the down removes only the exact row. List rows with the *case-insensitive* predicate so both show in the transcript. Pin it from a test: count the predicates on the name column with a pattern that does not require identifier quoting (and does not match sibling columns such as `custom_name` / `name_type`), and require the same count of exact ones.

**The runner never re-runs an applied migration, so it cannot show idempotency.** Pipe the up file into the database client a second time and compare a row listing (id, the shape columns, length and hash of the description) before and after. On a shared dev database check `status` first: a *shipped* migration another walk rolled back is applied together with yours, and the default rollback reverts the whole batch — roll back by name.
