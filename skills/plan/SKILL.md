...
- [references/ENDPOINT_BEHIND_A_RETRYING_QUEUE.md](references/ENDPOINT_BEHIND_A_RETRYING_QUEUE.md) — load when a plan adds or changes an endpoint whose caller is a queue worker (async-HTTP task, webhook redelivery): read the consumer's done / retry / give-up semantics from its code (200-only, body unread, unlimited retries turn a 4xx for a permanent error into an endless loop); map every outcome to done or retry in one pure function (permanent → 200 + `success:0`, retry-fixable → 403 / 5xx); the framework's pre-dispatch paths (unknown action, DB-down `die`, top-level catch) often answer 200 and silently complete queued calls — deploy the consumer before the producer's switch, switch off before rollback, DB-down → 503; a credential in the queued URL is frozen at enqueue, so key rotation loops on 403; a swallowing helper must report its outcome; write the residuals (terminal transport failure, serial-drain stalls, retried-older-value ordering) and amend the house status rule with a general exemption
