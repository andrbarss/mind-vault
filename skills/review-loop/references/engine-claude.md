...
- **Both tools resolve `<PR_NUMBER>` against the repository of the current working directory** (`gh` default-repo semantics). Run them with the cwd inside the *target project's* checkout (a worktree is fine); invoked from the mind-vault checkout they answer for mind-vault's PR of the same number — the finder then reports "no activity yet" for a target PR that is mid-review, and the retrigger fails with *Could not resolve to a PullRequest*. Field-observed on a downstream loop: a running review was read as NOT_TRIGGERED until the Actions API was queried directly. When a finder says "nothing found", confirm against the API before recording it (the same discipline as [`TOOL_INVOCATION_INTERPRETER.md`](TOOL_INVOCATION_INTERPRETER.md)).
...
- Every **subsequent push auto-SKIPS** → no fresh verdict (a "Skipping review …" no-op, caught by `CLAUDE_NOOP_PATTERNS`). Verified PR #169: pushes `06b3a3b` + `16e6dd4` both skip-no-op'd after the first review on `7399749`.
  **The skip is not deterministic (field observation, 2026-09-02).** On one PR a docs-only push after a clean first review received a FULL re-review (which raised a new finding), while the very next fix push skip-no-op'd in ~2 minutes. Never predict which outcome a push gets: read the verdict by summary time vs head-commit time on every wake (the `CLAUDE_STALE_SUMMARY` guard), and retrigger only when the guard says no verdict exists for the head SHA. Second observation the same day, downstream PHP project: after a clean cycle-0 the PR's *reader contract was reversed* (two commits, ten code/doc files, three OpenAPI descriptions) and the `synchronize` run did a **full ~7-minute re-review** that ran the project's suite and drift check itself and posted a fresh clean summary — with the repo's mention workflow (`claude.yml`) still read-only, so an explicit retrigger would have been muzzled. Substantive pushes after a clean first pass appear to get re-reviewed; the loop still reads it structurally (run DONE + summary newer than the head commit + zero inline), never by expectation. See [`CONTRACT_REVERSAL_AFTER_CLEAN.md`](CONTRACT_REVERSAL_AFTER_CLEAN.md) for what such a push must carry.
