...
**Doc-heavy / IDEA PRs — know which pass this is.** The canonical chain for a PR that carries substantial docs (IDEA file, plan, index, devlog, guides) is **two review passes around the doc wrap**: `work → review-loop (deliverables) → /wrap (bare, --scope=docs) → review-loop (docs) → /wrap --scope=full | human merge` — the same sequence sprint-auto runs (S3+S4 → S5 → S6+S7). Pass 1 reviews the code while the IDEA is still `in-progress` (correct — it isn't complete yet); the bare `/wrap` then finalizes the docs and **structurally cannot reach merge**; pass 2 reviews the docs at their merged shape so doc-consistency findings land in-loop instead of as post-review drift. Mechanics + the two-pass model: [`skills/wrap/references/WRAP_BEFORE_REVIEW.md`](../wrap/references/WRAP_BEFORE_REVIEW.md). Two practical notes: (a) **single-pass shortcut** — when the doc surface is small (a frontmatter flip + devlog entry), wrapping docs *first* and running one review pass is a legitimate collapse of the chain; say which variant you are running in the hand-back so the next session doesn't re-review. (b) Under a **push-triggered engine that skips an already-reviewed PR** (claude — see [`references/engine-claude.md`](references/engine-claude.md) § Incremental review), pass 2 will NOT auto-run on the wrap push; fire that engine's explicit retrigger (`claude_retrigger.sh`) or the docs pass silently never happens. (Code-only PRs: skip — go straight to the loop.)

**Pre-flight — un-draft the PR.** `/work` opens PRs as **draft** so the push-triggered Claude engine doesn't bill a review on every WIP commit (see [`references/engine-claude.md`](references/engine-claude.md) § Push-triggered model). The review stage is where that flips: if `claude` is in `ENGINES` and `gh pr view <PR> --json isDraft -q .isDraft` is `true`, mark it ready (`gh pr ready <PR>`) **before** Phase 1 — a draft PR makes claude no-op (the run fires but posts nothing → reads SILENT), so its verdict is unavailable until ready. Bugbot/Copilot are unaffected by draft state. Skip the un-draft only if the user explicitly wants the PR to stay draft. **Sequence it after the last push has settled:** un-drafting within a minute of a push lets the push's `synchronize` run start against a ready PR and do a full review too — two billed reviews of one SHA (engine-claude.md § 2026-09-08). Wait for that run's `Skipped — … draft` comment (≈90 s) before `gh pr ready`; and if two head-SHA runs exist anyway, a *completed* one with a post-dated summary and zero inline findings is the verdict — don't hold RUNNING on its twin.
...
4. **Decision tree — evaluate in order** (every `ScheduleWakeup(Ns)` below is shorthand for the full mandatory-prompt form from step 1 — `ScheduleWakeup(delaySeconds=N, prompt="/review-loop <PR_NUMBER> <ENGINES>")`; the `prompt` arg is never optional):
   ...
   - **Any engine `NOT_TRIGGERED`** → **first** `gh pr view <PR> --json mergeable` — a `CONFLICTING` PR gets **zero** `pull_request` workflow runs (engines *and* the test workflow), so the fix is a forward-sync push, not a retrigger (see [`references/CONFLICTING_PR_NO_CI.md`](references/CONFLICTING_PR_NO_CI.md)). Otherwise fire its retrigger (Phase 1 zero-activity path), set it `TRIGGERED`, `ScheduleWakeup(180s)`.
...
- [`references/CONFLICTING_PR_NO_CI.md`](references/CONFLICTING_PR_NO_CI.md) — a `CONFLICTING` PR (base moved under the branch) produces zero `pull_request` workflow runs for every engine and the test workflow alike; no failed run, no check. Check `gh pr view --json mergeable` before any retrigger on a push that fired nothing; forward-sync the base and push — never `close`/`reopen`.
...
- [`references/VENDORED_UI_SHELL_REVIEW.md`](references/VENDORED_UI_SHELL_REVIEW.md) — a PR that adds a vendored third-party UI shell (Swagger UI, ReDoc, a renderer, a dashboard) can read CLEAN on the diff while the risk sits in the bundle's *defaults* (validator badges that phone home with the tenant host, telemetry, CDN fallbacks, URL-fetch features). Dispatch the independent pass with its probe checklist; two small guard tests make the next bump a red/green signal.
...
- [`references/VERIFY_BOT_API_CLAIMS.md`](references/VERIFY_BOT_API_CLAIMS.md) — false-positive triage: a bot can be confidently wrong about a framework API when the project uses a non-default variant (toolkit/flavour/fork/pinned version). Verify the claim against the installed package (`node_modules/`/`vendor/`) before applying — current-symbol-found + suggested-replacement-absent is the tell, a green real-build run is the clincher. Refute on-thread, don't apply, don't re-trigger unchanged.
- [`references/TOOL_INVOCATION_INTERPRETER.md`](references/TOOL_INVOCATION_INTERPRETER.md) — the finders are `#!/bin/bash`; invoked as `zsh tools/<name>.sh` they degrade **silently** to a plausible "no activity yet" while the PR has a completed run and posted findings (bash-only `[[ =~ ]]`/`BASH_REMATCH` evaluate empty under zsh rather than aborting). Read as "no findings", that is a false CLEAN. Run `./tools/<name>.sh` or match the shebang; confirm any "nothing found" against the API before recording it.
- [`references/MERGED_MID_LOOP.md`](references/MERGED_MID_LOOP.md) — a human merging while the loop is still running strands every later commit: the merge commit's **second parent** is an earlier commit than the branch tip, yet the PR reads `MERGED`, the branch reads pushed, and the scratch file's `last_push_sha` is correct-and-irrelevant. Nothing in the usual signal set compares *what merged* against *what exists*. Detection is one `git merge-base --is-ancestor` call; recovery is to **measure the delta before proposing a revert** (field case: revert-and-re-land ~1170 lines vs ship-the-delta 9 files). Read when a post-merge state looks wrong, and before declaring an IDEA shipped.
- [`references/CONTRACT_REVERSAL_AFTER_CLEAN.md`](references/CONTRACT_REVERSAL_AFTER_CLEAN.md) — the user reverses a *contract-level* decision (a reader rule, a wire field, a default) after the loop read CLEAN. It is not a fix cycle: revise in place on the same PR, invert every pin / description / contract / header that encoded the old rule, re-run the runtime verification, refresh the PR body, and read the next verdict as a fresh cycle-0 — with a hand-off note when the old rule came from another repo's contract.
