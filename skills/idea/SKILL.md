...
- **Scan sibling branches too, not only the checked-out tree.** An IDEA that another branch has already moved to `docs/archive/` (its `/plan` ran on a `docs/idea-NNN-*` branch that has not merged yet) is invisible to the on-disk scan, so max + 1 collides with it. Before taking the number, list the IDEA files on every unmerged idea / feature branch: `git fetch -q origin; for b in $(git for-each-ref --format='%(refname:short)' refs/remotes/origin/ | grep -E 'idea-[0-9]+'); do git ls-tree -r --name-only "$b" docs/ideas/ docs/archive/; done | grep -oE 'IDEA-[0-9]{3}' | sort -u`. Field case: a capture re-used a *shipped* idea's number (the on-disk scan ran on a checkout that predated two more ideas sitting on unmerged branches) — the file was renumbered at `/plan` (`git mv`, a renumber note in the status line, an index fix). Cheap at capture time, a three-file rename afterwards.
- **Scan the default branch's tree, not the working tree.** A local `main` / `master` can sit many merges behind `origin/<default>`, and every idea shipped in that window is invisible to a glob of the checkout — the sibling-branch scan above does not catch it either, because the collision is on the default branch itself. Field case: a capture on a checkout fourteen merges behind took the number of an idea shipped a week earlier; `/plan` renumbered it across the archive dir, file, id, index, branch, worktree and PR, and the two sibling repositories that had already cited the old number needed a correction. Read the on-disk half from the remote: `git fetch -q origin && git ls-tree --name-only origin/<default> docs/ideas/ docs/archive/ | grep -oE 'IDEA-[0-9]{3}'` — or fast-forward the local default branch first; never trust `ls docs/…` on a checkout you have not just synced.
- Take max + 1 across **all** of the above. If no files exist, start at `IDEA-001`.
