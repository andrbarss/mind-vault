...
7. **Kill the process TREE, then verify the resource is free.** `pkill -f '<wrapper-pattern>'` retires only processes whose command line matches the pattern — a wrapper's CHILDREN (a java build daemon, a spawned sub-server, a vendor CLI re-exec'd under a different binary) often have argv that matches nothing in the pattern and survive the kill. A surviving child that holds a workspace lock or a port poisons every subsequent run of the same tool, and the next run's symptom misdirects (it "times out waiting" — which reads as slowness, not as lock contention). The kill discipline is two-step: (a) kill the wrapper AND its known child patterns (`pkill -f <wrapper>; pkill -f <child-daemon>`), then (b) **verify** the contended resource is actually free — `lsof -nP -iTCP:<port> -sTCP:LISTEN` for ports, the daemon's own "already running" log line for locks. A kill without the verify step is a hope, not a cleanup.
...
**E. Surviving child daemon holds the workspace lock — every later run wedges with a misdirecting timeout.** A dev-server stack (node webpack wrapper spawning a vendor build daemon: `java -jar …/sencha.jar app watch …`) was retired with `pkill -f webpack`. The node wrapper died; the java watch daemon — whose argv contains neither "webpack" nor the npm script name — survived, holding the build-tool's per-workspace watch lock. Every subsequent dev-server start logged `App watch is already running for this build profile` (visible ONLY in the server's own log) and never finished its first bundle, surfacing to the test harness as a `webServer` startup timeout — which misdirected diagnosis toward "cold compile too slow" (the true clean cold start measured ~24s, well within budget). Stacked retries added more lock contenders, never a winner. Fix per Hard Rule 7: kill both the wrapper AND the daemon pattern, then verify the port/lock is free before the next start; when a server-start timeout appears, grep the server's own log for an "already running" line BEFORE reaching for timeout bumps.
...
Hard Rule 7 + Failure Mode E added 2026-06-11 from an ExtJS/Sencha dev-server session: an incomplete `pkill` left a vendor build daemon holding a workspace lock, and the resulting server-start timeouts misdirected diagnosis for three debugging rounds before the daemon was found.
