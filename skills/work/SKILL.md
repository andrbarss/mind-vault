...
- [references/CAPTURE_FIRST_API_DOCS.md](references/CAPTURE_FIRST_API_DOCS.md) — load when the item is "document existing endpoint(s)": schema from a captured response ⊕ the DDL (never the DDL alone), a per-endpoint verification guide as the acceptance checklist, throwaway-row probing, structural guards, the recurring traps (0-keyed map example, never-empty loaders, sibling-branch asymmetry, the key that is null in every capture = a dead-column finding, examples that cannot show a fix, the tokenised multi-site pin)
...
- [references/EXECUTE_OVER_PIN.md](references/EXECUTE_OVER_PIN.md) — load when a step amends a method the DB-free suite cannot construct and its only coverage is a source-level pin: a pin proves text, not behaviour — bypass the constructor, seed the collaborator cache, trace every exercised branch away from the adapter, and assert the exact output with inputs that discriminate the semantics claimed; keep the pin for the shape; when the contract is a *sequence* (validate → write A → read back → write B → side effect), move it behind a small gateway interface and test it with a scripted fake — a verification step that needs a patched build is the tell; a pin over a value whose meaning you inferred from a library option's *name* certifies the belief — execute the library's own test double with a positive control of the quirk, and ask of every walk row whether the environment could have produced the failing input; a second environment cloned from the first hides every cross-environment lookup — mark the copy, add ids the other side lacks, use fresh environment identifiers, keep the failing capture; one predicate behind N inline compares — the legacy branch evaluates the legacy expression (quirks included), a missing column is legacy, and when two legacy sites used different forms pin the row where they disagree
- [references/CLI_SEED_CHARSET_HIDES_MOJIBAKE.md](references/CLI_SEED_CHARSET_HIDES_MOJIBAKE.md) — load when a verification walk seeds non-ASCII text through a DB CLI and captures a response carrying it: a client started without `--default-character-set=utf8mb4` double-encodes the bytes, the same session's `SELECT` transcodes them back and shows exactly what you typed, and only the wire (or `HEX()`) reveals `Å³`; seed with the charset set, verify on the wire never through the writing session, quote docs from the capture not the CLI
- [references/CROSS_SPEC_UI_LEAKAGE.md](references/CROSS_SPEC_UI_LEAKAGE.md) — load when a green in-browser suite turns intermittently red after a new spec file is added and the failure is in a *different*, pointer- or geometry-driven spec: a floating artefact the component does not own (a toast, a wait box, a mask, focus, scroll) outlives its spec and only bites when the leaking file runs first under random order, and no single row reproduces it; A/B the suite with and without the new file, fix the order with the suspect first, bisect by `describe` then `it`, stub the artefact and assert it fired — never pin suite order; three non-visual shapes of the same class — a fixture object the framework adopts as live record data, an orphan component with global listeners (a real production bug the suite reached first), a route pushed by a save that closes its window — and survivor listing + a raised `Error.stackTraceLimit` to find a thrown one
- [references/MUTATION_PASS_DISCIPLINE.md](references/MUTATION_PASS_DISCIPLINE.md) — load when a plan or review asks for mutation checks, or after a fix cycle adds guards: one mutation per guard, planned first (it exposes status-only assertions and data-provider rows that cannot discriminate), run against committed code one at a time with a git restore and an unmutated baseline in the same script, never in the same parallel batch as a staging command; a run that dies on the same fatal every time is void, and a mutation that reproduces its defect can leave debris to clean by the defect's own signature. The same test applies to a manual smoke: it must be able to fail on the unfixed code. Force the diverging condition with a hand-placed fixture. A timing race counts only when the fix's own tell appears; otherwise it is inconclusive.
- [skills/plan/references/CONTRACT_CONSUMER_DISCIPLINE.md](../plan/references/CONTRACT_CONSUMER_DISCIPLINE.md) — load when the plan being executed was coded against another codebase's plan-stage contract: re-read the contract at the end of `/work` (its owner amends it as their code lands — a changed envelope root key opens the edit window empty and no proxy-stubbing spec can catch it), verify the shapes the runtime walk cannot reach against the producing code, and never let a `[]`-means-clear key serialise from a not-yet-loaded reference list — nor ride along from a failed earlier save once the gate has closed
