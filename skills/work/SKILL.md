...
- [references/GUARD_MUTATION_CHECK.md](references/GUARD_MUTATION_CHECK.md) — load at the end of a step that added guards and the specs pinning them: break one guard at a time in a committed file (backup, one substitution, confirm it applied, count the suite, restore); every mutation must fail at least one spec, and a green one names a vacuous row; run it alone — never while committing or while a reviewer reads the working tree
