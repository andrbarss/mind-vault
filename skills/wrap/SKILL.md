...
**Forward-sync the default branch first (pre-merge mode).** `git fetch origin && git log --oneline HEAD..origin/<default>` — if anything landed since the branch was cut, `git merge origin/<default>` into the feature branch, resolve (shared docs conflicts are almost always keep-both in ship order), run the suite, commit the merge, and only then run Steps 2–6. The wrap edits the files every IDEA edits (ideas index, devlog, `CLAUDE.md`, architecture/how-to docs), so skipping this surfaces a `CONFLICTING` PR at the human's merge click, after review cleared. Also grep the modules the merge brought in for any convention this IDEA introduced — a sibling authored in parallel can't know about it; record gaps, don't silently patch code in a docs pass. Mechanics + checklist: [`references/PRE_WRAP_FORWARD_SYNC.md`](references/PRE_WRAP_FORWARD_SYNC.md).
