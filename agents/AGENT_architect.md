...
- **Read-path normalisation hides stored-data breakage** — when a plan puts trim / lower-case / coercion of a user-editable value into the **deserialisation** layer (an ORM/model field `convert`, a serializer's `to_representation`, a getter/accessor), every read surface — list, detail, picker — shows the *canonical* form, and a stored value that violates the rule (leading space, wrong case, legacy hand-inserted) becomes invisible in exactly the admin screen built to expose it; it also silently breaks any contract that says "the stored value is echoed unchanged". The tell in the plan: normalisation described as a model/field concern rather than a save concern, or a grid renderer adding encoding "because legacy values are echoed raw" while the model has already rewritten them. Demand (1) normalisation at the **write seam only** (form → record → request), with **one** shared normaliser producing both the "did it change?" comparison and the value sent, so the two cannot disagree; (2) a faithful read path (no `convert`; at most a trim if the contract permits it); (3) a verification row that **hand-inserts a non-canonical stored value** and asserts the UI shows it verbatim — a probe that only round-trips through the UI's own save can never observe this, because the save canonicalises before the read. Sibling of phantom verification: the round-trip probe is green on both the faithful and the rewriting implementation.
...
- **A response-shape change produced in code flips at deploy, not at migrate** — the deploy-order sibling of the two bullets above. When a plan changes the *shape* of a field (list → keyed object, scalar → struct, a different empty form) and the transformation lives in the shaper / serializer rather than behind a schema gate, the new shape reaches **every** deployable the moment the code lands — including tenants not yet migrated, whose rows simply flow through the new shaper with defaults. The migration gate the plan already carries (fleet `status` before deploy) stages nothing here; there is no server-side window in which both shapes exist. Demand the plan state this in its deploy gate and pin a **consumer-first order**: (1) inventory the readers of the field (grep the repo and name the external consumers explicitly — "none in this repository" is a claim to verify, not assume), (2) consumers become shape-tolerant (`Array.isArray(x) ? x : Object.values(x)` or the new read) and deploy first, (3) the producer deploys, (4) the schema migration lands whenever — the degrade ladder covers both orders. A plan that offers a compatibility switch or a parallel key instead must say who removes it and when; a plan that offers neither must say the break is deliberate and recorded in the amended IDEA's archive backref.
