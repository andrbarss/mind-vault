...
- **Refusal-origin blindness in hostile-input tests**: a test that sends a malformed shape and asserts only the status code (`400`, `422`) is green for the wrong reason whenever the fixture also cites a parent id / foreign key that does not exist — delete the parser rule and the request is *still* refused, one layer later, by the existence check. Look for a data provider whose rows all carry `<parent>_id => 1` against a live DB, and a consumer that reads nothing but the code. Require the assertion to pin the refusal's **origin** (the message text or the field the parser names, or `assertStringNotContainsString('unknown …')`), and check the transport can carry the distinction under test at all — a JSON encoder that serialises `1.0` as `1` makes an HTTP-level "float is refused" row unfalsifiable; pin that one at the service layer instead. Same family as a degrade double that reports absence while the columns exist: name the one-line production edit that turns the test red, or it is a decoration.
- **A guard masked by a framework fallback**: when a guard's observable effect — "no SQL names the column", "the key is absent from the payload", "the row is not written" — has a **second producer** (an ORM that silently drops keys naming a column the table lacks, a DB default, a null-tolerant reader, a sibling guard upstream), a test at the HTTP / response surface cannot tell whether the guard ran. Field case: a degrade strip shipped **dead** and thirty-two route tests, three with the column genuinely dropped, stayed green because the ORM did the strip's job; the docblock said the code did not rely on that internal. Demand the pin at the guard's **own** return value or state write (call the decision method directly; `assertArrayNotHasKey` on the payload it hands back), and ask for the one-line edit *inside the guard* that turns the test red — an edit in the framework does not count. The mechanism to grep for while you are there: a **by-reference call inside an array / tuple literal, an argument list or a concatenation that also reads the same variable** (`[$x, byRef($x)]`) — the language copies the value before the call mutates it, so the mutation never reaches the returned structure; the result must be assigned to a local first.
- **Provenance leak in consumer-facing artefacts**: when the diff touches text that ships to outsiders (an OpenAPI / Swagger document, generated reference docs, a public changelog, CLI help), grep the *generated output* for sprint vocabulary — idea / decision / PR ids, "captured", "code-read", "verified", "observed", "probe", "on the dev stack / clone", missing-extension errors, "(redacted)". One hit is a finding of the same class as a hard-coded local path; the artefact describes the thing, never how it was documented. Mechanics + guard test: `skills/work/references/GENERATED_ARTEFACT_HYGIENE.md`.
...
- **Numeric-string gates — finiteness and a ceiling**: a gate that accepts "a positive number" through `is_numeric()` / `parseFloat()` / `float()` accepts `1e999` — an infinity that passes `> 0`, `round()`, and every `< minimum` comparison and reaches the INSERT (stored as `0` under a lax SQL mode, a 500 under a strict one) — and `1e20` past the DECIMAL column. Demand `is_finite()` / `Number.isFinite()` / `math.isfinite()` plus a ceiling that fits the column at the ONE gate the decision and the writer share, and test `1e999`, `NAN`, twenty digits, ceiling + 1 cent. Mechanics: `skills/plan/references/ROUTE_A_VARIANT_THROUGH_A_SHARED_WRITER.md` § 5.
- **Credential-change revocation parity**: every path that changes or removes a credential — password change/reset, account deactivation, role or permission drop, API-key rotation — must revoke the SAME access artefacts (server sessions, remember-me / refresh tokens, API tokens, device cookies). Line the paths up as a family: if `deactivate()` purges sessions and rotates the remember token but `changePassword()` only rewrites the hash, an admin resetting a compromised account leaves the attacker logged in — a rotation that isn't a revocation. Bots reviewing the diff read each handler as locally correct; only the side-by-side truth table exposes the odd one out. Demand a shared `revokeAccess()`-style helper called from every member, with the caller's own current session exempted when a user acts on their own account. Lock with a test per path that seeds a live session for the target AND a bystander and asserts only the target's is gone.
...
- **Cross-clock cutoffs (silent data destruction)**: when a query compares a timestamp column against a cutoff, check *which clock writes that column*. A column defaulted by the database (`CURRENT_TIMESTAMP`, `now()`) compared against a cutoff computed in application code is a cross-clock comparison — and app runtimes are routinely pinned to a local zone while the DB session runs UTC. Wherever the two differ, an expiry / reaper / retention sweep destroys rows that are still live, and no test catches it because both clocks agree in CI. Demand the cutoff be evaluated on the writing clock (`created_at < DATE_SUB(NOW(), INTERVAL :n MINUTE)`), with only a validated integer interval crossing the boundary.
- **Multi-column UPDATE assignment order**: a single-table `UPDATE`'s SET list is evaluated **left to right using already-updated values** (MySQL; other engines differ — check before porting), and ORMs generally emit the pairs in the order the caller supplied them. So when several assignments in one statement are conditional on the *same* discriminator column (`IF(status_by = :marker, …)`), that discriminator must be assigned **last** — put it first and every following condition tests the new value and silently never fires. Reject a review suggestion to "simplify" such an ordering unless the reviewer has shown the engine's evaluation rule; the failure is a no-op that only a test asserting the post-state of *all* the columns together will catch.
