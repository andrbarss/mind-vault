...
- **A source-level pin on an amended method is not a behavioural test.** When a method the DB-free suite cannot construct (an ORM/table-gateway model, a framework controller) is *changed* — an extraction, a delegation, a refactor on a write path — and its only coverage is a reflection pin on the method's source (strings present, order, absence), the suite proves the text, not that a given input still yields the same result. Ask *"does any test call it?"* If not: construct the object without its constructor (`ReflectionClass::newInstanceWithoutConstructor`, `Model.__new__`), seed the collaborator cache the method reads, trace every exercised branch to confirm none reaches the adapter, and assert the exact output — with inputs that discriminate the semantics claimed (`'10'` not `'4'` to prove a numeric comparison; the exact min and max for inclusivity). Keep the pin for the shape. Full recipe: [`../skills/work/references/EXECUTE_OVER_PIN.md`](../skills/work/references/EXECUTE_OVER_PIN.md).
