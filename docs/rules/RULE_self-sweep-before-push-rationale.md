...
7. **Negative-existence claims are grep-verified before they land in a doc.** Any sentence of the shape "`<framework>` has no `<method>`", "`<library>` doesn't support X", "there is no `<helper>` for Y" is the *easiest* claim class to falsify and the *most likely* to be wrong: it usually originates in an agent spike that stopped at the first failing attempt (the method lived on a parent class the spike never grepped), and — because a negative reads as a settled fact — it propagates unchallenged. Field case: a plan's spike note "the test-request class has no `setQuery()`" (the method existed, inherited from the parent request class; `setParam()` had simply been the first thing that worked) was copied verbatim into the test's header comment, the archive README, a cross-idea backref and the plan's resolved open question — four places, three commits, two agents — before an independent claim-vs-code reviewer grep-falsified it. The check is one command per claim, run against the *installed* code, not memory:
   ...
   Two more shapes of the same claim, both field-observed in one sprint. **A novelty claim** — "found on the way", "new", "nobody documented this" — asserts that the subject exists *nowhere in the project's own docs*; it is cheap to write and flattering to report. A verification walk noticed that a wholesale endpoint published credential rows, a backlog item and a PR section called it a finding — and the wrap's downstream scan turned up a shipped contract, eleven days old, that already told consumers never to forward that map. What was new was narrower (no backend-side fix had been filed; a specific flag was measured). `grep -rn '<subject keyword>' docs/` before writing "found", and then say exactly what is new. **A negative about a sibling repository** — "nothing writes column X", asserted about a table two codebases write. It passed a plan review, an engine and two independent reviewers, each of whom could see one repository; one `grep` in the other showed its model mapping the framework's `updated_at` onto that very column. "Say which root you searched" applies across repositories: open the other one, or write "not checked there".
