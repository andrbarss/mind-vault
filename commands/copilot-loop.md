...
> - Clean-signal phrasing: ✅ confirmed (2026-05-27). A cleared review is `state: COMMENTED` with body `"Copilot reviewed N out of N changed files in this pull request and generated no comments."` — and the post-retrigger variant `"…generated no new comments."` — plus ZERO entries in `/comments`. Match on "generated no comments" / "no new comments", not an exact string.
>
...
>
> **Refinement (2026-05-27 empirical).** The error body has **multiple phrasings** — also seen: `"Copilot wasn't able to review any files in this pull request."` Treat both as the same error class. And the failure is often **transient and recovers on the 3rd attempt**: a run hit error → (different-phrasing) error → **clean real review** on the 3rd retrigger. So the "2 consecutive → stop" guard is conservative; making ONE more attempt after two errors frequently recovers. Reserve the hand-back for **3** consecutive failures, not 2. (Reviews also lag longer than first thought — **5–9 min** observed; poll with a ~9–11 min window before concluding "no verdict".)
...
> **Behaviour — re-scan non-determinism (2026-05-27 empirical).** Copilot re-scans **all** changed files on every retrigger and is non-deterministic per scan: it surfaces *new, previously-unflagged* findings on **unchanged, already-reviewed** code across successive rounds — one PR's docs-pass went **4 → 1 → 2 → 0** findings over consecutive clean-up cycles, each round's findings genuinely new and mostly legitimate (test-assertion robustness, a stale doc line, a null-guard). Implications: (a) a clean PR can take several retrigger rounds even when you're only fixing real issues — **stop on the first clean round, don't chase the tail indefinitely**, and tell the user the PR is functionally solid if the tail is just diminishing nits; (b) round-N *new* findings are NOT a no-progress signal — the no-progress detector keys on the **same category repeating**, which this isn't; (c) docs-only / `/wrap` commits can themselves draw fresh findings on re-scan, so one post-`/wrap` retrigger round is worth running before final hand-back.

> **Refinement — service-error mode is NOT SHA-cached; wait IS the lever (2026-05-28 empirical).** During one PR's loop, Copilot's service entered the error mode (`"Copilot encountered an error..."`) and stayed there across **5 consecutive retriggers spanning ~3 hours**, including across a deliberate **kick-commit on a NEW SHA** (push a trivial change to give Copilot a different commit to look at). Service-error persisted on the new SHA identically. Conclusion: the failure isn't SHA-cached at Copilot's end, so new-SHA / kick-commit retries are **NOT** the lever. The existing "3 consecutive errors → hand back" rule is correctly broad. The lever that worked: **wait and retry later** — a single retrigger ~3 hours after the streak started returned a clean real review. Pragmatic guidance to layer on top of the 3-error hand-back: when the hand-back fires, tell the user to retry from the GH UI later (windows here ranged 30 min – 3 hr); don't burn cycles cycling SHAs.

> **Behaviour — retroactive `/copilot-loop` on sprint-auto v3.2 per-IDEA PRs is a no-op (2026-05-28 empirical).** Sprint-auto v3.2 routes per-IDEA PRs with `--base $SPRINT_AUTO_INTEGRATION_BRANCH`. The instant the integration phase's sequential merge (S11.6) lands a per-IDEA branch's tip into the integration branch, GitHub marks that per-IDEA PR as **`MERGED`** (its head is reachable from its base). Copilot won't review merged PRs — `/requested_reviewers` POST is silently dropped; `/reviews` stays empty. So invoking `/copilot-loop` retroactively on a per-IDEA PR after the batch's integration phase is **definitionally a no-op**, and the agent should detect-and-skip immediately rather than poll. The reviewable surface for a retroactive post-batch run is the **`[INTEGRATION]` PR (S11.10)** only — it targets `main` (still OPEN until the human merge) and carries the union of all per-IDEA diffs. If the operator says "run `/copilot-loop` on PRs #N, #M, #integration" post-batch, the right behaviour is: skip #N and #M (already merged into integration, Copilot won't review), run only on #integration.
